...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, max_seq_len: int, dropout: float) -> None:
        ...
        # We know that  a^-x  is equals to 1/a^x
        frequency = torch.pow(
            10000, -torch.arange(0, d_model, 2, dtype=torch.float)/self.d_model)

        pe = torch.zeros((max_seq_len, d_model))
        pe[:, 0::2] = torch.sin(pos * frequency)
        pe[:, 1::2] = torch.cos(pos * frequency)

        # Here we use register_buffer inorder to avoid update model parameter during backpropagation
